function init (json) {
    const article = `@title{Getting Started With React Redux: An Intro}
    ...
    document.getElementById('body-container').innerHTML = translate(string)


    $('pre code').each((i, block) => {
       hljs.highlightBlock(block)
     })
    log(document.getElementById('title').innerHTML)
    return 10
}

function log (string) {
    $('#debug-container').text(string)
}
